 ...
 **/
contract SDRXICO is Ownable {
  using SafeMath for uint256;
  Token token;

  uint256 public tokenRate = 740; // Number of tokens per Ether 739.32
  uint256 private totalTokensSold = 0;    // total tokens sold in  ICO
  uint256 public constant Hard_cap = 1000000000; // Cap in Ether
  uint256 public constant Soft_cap = 100000; // Cap in Ether
  uint256 public constant icoStart = 1569395859;//  25  September , 2019 @ 12:47:39 PM GMT+05:30
  uint256 public icoEnd = 2200547859 ;//Sunday, 25 September 2039 12:47:39 PM GMT+05:30
  ...
  function calculateBonus() internal view returns(uint) {
       ...
       if(totalTokensSold < 100000000){
        return 30;
      } else if(totalTokensSold < 500000000){
        return 20;
      } else if(totalTokensSold < 1000000000){
        return 10;
      } else {
        return 0;
      }
